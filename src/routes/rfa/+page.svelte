	...
	const fetchRFAs = () => {
		runQuery(queries['rfas'], {}).then(({ data }) => {
			switch (sortMethod) {
				...
				case 'posrank':
					data.contracts.sort(objByProperty.bind({ path: 'player.positionRankProj', dir: 'asc' }));
					break;

				case 'ovrrank':
					data.contracts.sort(objByProperty.bind({ path: 'player.overallRankProj', dir: 'asc' }));
					break;
			...
			const playerData = data.contracts.map((contract: Contract) => ({
